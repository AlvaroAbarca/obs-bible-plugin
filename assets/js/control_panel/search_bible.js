...
function searchBible(query, bible_data) {
  ...
  if (lowercaseQuery.includes("-")) {
    ...
    for (let i = startVerse; i <= endVerse; i++) {
      ...
      const matchingVerse = bible_data.find(
        (bible_data) =>
          bible_data.name.toLowerCase() === verseName.toLowerCase(),
      );

      if (matchingVerse) {
        savedBibleVerse.push(matchingVerse.ari);
        const pElement = document.createElement("p");
        const cleanedName = matchingVerse.name
          .replace(/:/g, "-")
          .replace(/\s/g, "")
          .toLowerCase();
        pElement.id = cleanedName;
        pElement.innerHTML = `${matchingVerse.verse} <span>${matchingVerse.name.toUpperCase()}</span>`;
    ...
    for (let i = 0; i < bible_data.length; i++) {
      ...
      if (name === lowercaseQuery || verse === lowercaseQuery) {
        const cleanedName = bible_data[i].name
          .replace(/:/g, "-")
          .replace(/\s/g, "")
          .toLowerCase();
        ...
        pElement.id = cleanedName;
        pElement.innerHTML = `${bible_data[i].verse} <span>${name.toUpperCase()}</span>`;
    ...
    for (let i = 0; i < bible_data.length; i++) {
      ...
      const isMatchInName = searchWords.every((word) =>
        fuzzySearch(word, name, (threshold = 1)),
      );
    ...
    for (let i = 0; i < bible_data.length; i++) {
      ...
      if (
        name.includes(lowercaseQueryWithColon) ||
        verse.includes(lowercaseQueryWithColon)
      ) {
        const cleanedName = bible_data[i].name
          .replace(/:/g, "-")
          .replace(/\s/g, "")
...
function getBibeAri(query) {
  ...
  if (query) {
    ...
    for (let i = 0; i < query.length; i++) {
      const matchingVerse = bible_data.find(
        (bible_data) => bible_data.ari === query[i],
      );
      if (matchingVerse) {
        savedBibleVerse.push(matchingVerse.ari);
        const pElement = document.createElement("p");
